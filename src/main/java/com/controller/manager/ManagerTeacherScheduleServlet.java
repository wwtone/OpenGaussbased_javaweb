...
@WebServlet("/AdminTeacherScheduleServlet")
public class ManagerTeacherScheduleServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (teacherScheduleList.isEmpty()) {
            // 如果没有查询结果，重定向到初始页面
            response.sendRedirect("manager_teacherSchedule.jsp");
        } else {
            // 如果有查询结果，将结果传递给JSP页面
            request.setAttribute("teacherScheduleList", teacherScheduleList);
            request.getRequestDispatcher("manager_teacherSchedule.jsp").forward(request, response);
        }
    }
